  ...
  Hooks.once("init", () => {
    ...
    // Assuming 'SomeClass' is globally available
    ...
    // Open module API
    window.modifyTokenAttribute = GURPSNegativeHP.modifyTokenAttribute;
  });

  class GURPSNegativeHP {
  
    static async modifyTokenAttribute(attribute, value, isDelta=false, isBar=true) {
      const attr = foundry.utils.getProperty(this.system, attribute);
      const current = isBar ? attr.value : attr;
      const update = isDelta ? current + value : value;
      if ( update === current ) return this;
